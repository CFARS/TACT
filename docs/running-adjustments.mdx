...
<Steps>
  ...
  <Step title="Run Adjustment">
    ...
    results = tact.adjust(
        data=data,
        method="ss-sf",  # or "ssws", "sswsstd", "baseline", "bat"
        parameters={
            "split": True,              # Use train/test split (not needed for bat)
    ...
    **Method options:**
    ...
    - `bat`: SSA-based correction (no training data needed)
  ...
  <Tab title="BAT">
    **Bias Adjustment Technique**
    ...
    results = tact.adjust(
        ...
        parameters={
            "config_path": "config.json",
            "L": 100,        # Optional: SSA window length
            "n_calib": 35    # Optional: Calibrated components
        }
    )
    ```
    ...
    **Best for:**
    ...
    - Pre-trained coefficient-based correction
    - SSA-based time series decomposition
    - Systematic bias correction

    **Note:** Requires `Generic_BAT35_wk6.pkl` coefficient file in `tact/assets/bat/`
  </Tab>
</Tabs>
...
| `RSD_adjWS` | Adjusted wind speed (SSWSStd, BAT) |
| `RSD_adjSD` | Adjusted std deviation (SSWSStd, BAT) |
