...
import numpy as np
import json
from tact.utils.post_adjustment_stats import post_adjustment_stats
from tact.utils.get_regression import get_regression
from tact.utils.train_test_split import train_test_split

def perform_SS_SF_adjustment_ported(data, parameters):
    ...
    with open(parameters["config_path"], 'r') as f:
        ...
        data = train_test_split(80.0, data.copy())
        
        inputdata_train = data[data["split"]].copy()
        inputdata_test = data[~data["split"]].copy()

        if data.empty or len(data) < 2:
            results = post_adjustment_stats(
                [None], results, "Ref_TI", "adjTI_RSD_TI"
            )
            if "Ane_TI_Ht1" in data.columns and "RSD_TI_Ht1" in data.columns:
                results = post_adjustment_stats(
                    [None], results, "Ane_TI_Ht1", "adjTI_RSD_TI_Ht1"
                )
            if "Ane_TI_Ht2" in data.columns and "RSD_TI_Ht2" in data.columns:
                results = post_adjustment_stats(
                    [None], results, "Ane_TI_Ht2", "adjTI_RSD_TI_Ht2"
                )
            if "Ane_TI_Ht3" in data.columns and "RSD_TI_Ht3" in data.columns:
                results = post_adjustment_stats(
                    [None], results, "Ane_TI_Ht3", "adjTI_RSD_TI_Ht3"
                )
            if "Ane_TI_Ht4" in data.columns and "RSD_TI_Ht4" in data.columns:
                results = post_adjustment_stats(
            ...
            if len(full) < 2:
                ...
                inputdata_test["adjRepTI_RSD_RepTI"] = (
                    RSD_TI + 1.28 * inputdata_test[column_map["rsd"]["primary"]["standard_deviation"]]
                )
            
                
                results = post_adjustment_stats(
                    inputdata_test, results, column_map["reference"]["turbulence_intensity"], "adjTI_RSD_TI"
                )
    ...
    # NOTES 4/16/2025
    # Need to figure out why output doesn't match 