...
class Data(Config):
    ...
    def get_inputdata(self):
        ...
        if 'RSD_TI' in self.inputdata.columns.to_list():
            ...
            if 'RSD_SD' in self.inputdata.columns.to_list():
                self.inputdata['RSD_TI'] = self.inputdata['RSD_SD']/self.inputdata['RSD_WS']
            else:
        ...
        self.inputdata['bins'] = self.inputdata['Ref_WS'].round(0)  # this acts as bin because the bin defination is between the two half integer values
    ...
    def check_for_alphaConfig(self):
        ...
        availableData = pd.read_excel(self.config_file, usecols=[1], nrows=1000).dropna()['Header_CFARS_Python'].to_list()
        if 'RSD_alpha_lowHeight' in availableData and 'RSD_alpha_highHeight' in availableData:
            self.RSD_alphaFlag = True
            configHtData = pd.read_excel(self.config_file, usecols=[3, 4], nrows=25).iloc[[20,21]]
